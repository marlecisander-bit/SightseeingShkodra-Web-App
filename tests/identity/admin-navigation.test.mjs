...
test('admin navigation matches the intended role workspaces', () => {
  assert.deepEqual(navigationFor('content_editor').map(item=>item.slug), ['overview','content']);
  assert.deepEqual(navigationFor('operations').map(item=>item.slug), ['overview','departures','bookings']);
  assert.equal(navigationFor('owner').length,5);
  assert.equal(navigationFor('admin').length,5);
});
