...
const users = [];
const sessions = [];
const conflictFixtures = { content_pages: randomUUID(), departures: randomUUID(), products: randomUUID() };
function ok(result) {
...
try {
  ...
  // SQL tests alone miss PostgREST's automatic serialization_failure retries.
  // Use direct synthetic fixture inserts; stale RPCs must never write or append audit events.
  const operator = operators[0];
  const actor = ok(await admin.from('staff_profiles').select('id').eq('operator_id', operator).eq('auth_user_id', users[0]).single()).id;
  ok(await admin.from('content_pages').insert({id:conflictFixtures.content_pages,operator_id:operator,title:'Conflict fixture',slug:'conflict-fixture'}));
  ok(await admin.from('products').insert({id:conflictFixtures.products,operator_id:operator,title:'Conflict fixture',slug:'conflict-fixture',type:'van_tour'}));
  ok(await admin.from('departures').insert({id:conflictFixtures.departures,operator_id:operator,product_id:conflictFixtures.products,service_date:'2030-06-01',start_time:'09:00',capacity:2,status:'draft'}));
  const conflicts = [
    ['content_pages','save_content_page_v1',{p_operator_id:operator,p_actor_id:actor,p_id:conflictFixtures.content_pages,p_data:{title:'Must not persist',slug:'conflict-fixture',status:'draft',text:'stale'},p_expected_updated_at:'2000-01-01T00:00:00Z'}],
    ['departures','save_departure_v1',{p_operator_id:operator,p_actor_id:actor,p_id:conflictFixtures.departures,p_product_id:conflictFixtures.products,p_vehicle_id:null,p_date:'2030-06-01',p_time:'09:00',p_capacity:1,p_status:'draft',p_expected_updated_at:'2000-01-01T00:00:00Z'}],
  ];
  for (const [table,rpc,args] of conflicts) {
    const before=ok(await admin.from(table).select('*').eq('id',conflictFixtures[table]).single());
    const result=await admin.rpc(rpc,args).abortSignal(AbortSignal.timeout(5000));
    assert.equal(result.error?.code,'PT409');
    assert.equal(result.status,409);
    assert.deepEqual(ok(await admin.from(table).select('*').eq('id',conflictFixtures[table]).single()),before);
  }
  console.log('PASS: hosted stale content/departure edits return HTTP 409 within the deadline without changing records');
  ...
  for (const [table,id] of Object.entries(conflictFixtures)) {
    const result=await admin.from(table).delete().eq('id',id).eq('operator_id',operators[0]);
