...
Date: 2026-09-21. Result: **PASS for Phase 1 foundations**. Checkpoint: `checkpoint-phase-1-foundations` after verification and commit.

Final acceptance supersedes the earlier pending-platform notes below: see PHASE_1_PLATFORM_ACCEPTANCE.md. The actual proxy now passes SSR cookie refresh, invalid-cookie and rejected-refresh checks against hosted Auth. A privileged hosted RPC successfully records and replays an immutable synthetic event/audit pair. Browser/admin-screen end-to-end tests remain future-phase work, not a claim of this foundation checkpoint.
...
| Staff roles and verified authorization | PASS | 12 identity tests plus hosted sign-in, real proxy cookie refresh and rejected-refresh session removal; see PHASE_1_PLATFORM_ACCEPTANCE.md for simulation limits. |
| Operator-scoped RLS | PASS | All-role database tests plus real hosted two-operator isolation, deactivation and direct write/RPC denial. |
