...
function Editor({entity,row,operatorId,products,suppliers}:{entity:string;row:Row;operatorId:string;products:Row[];suppliers:Row[]}) {
  return <form action={saveCatalog.bind(null,operatorId,entity)} className="catalog-form">
    ...
    <button type="submit" name="operation" value="save">{row.id?'Save changes':'Create'}</button>
    {row.id && <div><label><input type="checkbox" name="confirm_remove" value="yes"/> Confirm {entity==='product'?'archive':'deletion'}</label><button type="submit" name="operation" value="remove" formNoValidate>{entity==='product'?'Archive product':`Delete ${entity}`}</button></div>}
  </form>;
}
